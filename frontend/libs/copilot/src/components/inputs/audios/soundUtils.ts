...
export type ProcessFrequencyDataProps = {
  data: Uint8Array;
  canvasRef: RefObject<Canvas> | RefObject<HTMLCanvasElement>;
  barWidth: number;
  gap: number;
...
export const processFrequencyData = (process: ProcessFrequencyDataProps): void => {
  if (!process.canvasRef.current) return;

  const dataPoints = calculateBarData(
    process.data,
    process.canvasRef.current.width,
    process.barWidth,
    process.gap
  );
